  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-50/90 backdrop-blur-sm">
      <div className="flex flex-col items-center space-y-4">
        <ImSpinner2 className="h-10 w-10 animate-spin text-brand-650" />
        {message && (
...
export const Spinner = ({ size = 'md', className = '' }) => {
  ...
  return (
    <ImSpinner2 className={`animate-spin text-brand-600 ${sizes[size]} ${className}`} />
  );
};
  ...
  return (
    <div className="space-y-3 w-full animate-pulse">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className={`bg-slate-200 rounded-lg ${className}`} />
      ))}
    </div>
  );
};
