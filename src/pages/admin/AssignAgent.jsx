  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <Link
          to="/admin/orders"
          className="inline-flex items-center text-xs font-semibold text-slate-500 hover:text-slate-755 dark:text-slate-400 dark:hover:text-slate-205"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-205 dark:border-slate-750 shadow-card space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-800 dark:text-white uppercase">{order.pickupZone}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Drop Zone:</span>
                <span className="font-semibold text-slate-800 dark:text-white uppercase">{order.dropZone}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Billable Weight:</span>
                <span className="font-semibold text-slate-805 dark:text-slate-100">{order.billableWeight} kg</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Payment mode:</span>
                <span className="font-semibold text-slate-805 dark:text-slate-100">{order.paymentType}</span>
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-205 dark:border-slate-750 shadow-card space-y-3.5">
            <h3 className="text-xs font-bold text-slate-850 dark:text-slate-200 uppercase tracking-wider">
              Route locations
            </h3>
            <div className="space-y-3 text-xs leading-relaxed">
              <div>
                <p className="font-bold text-slate-700 dark:text-slate-350">Pickup:</p>
                <p className="text-slate-500 mt-0.5">{order.pickupAddress}</p>
              </div>
              <div className="pt-2">
                <p className="font-bold text-slate-700 dark:text-slate-350">Drop Destination:</p>
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          {nearestAgents.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 p-8 text-center border rounded-xl text-xs text-slate-500 leading-relaxed">
            ...
            <div className="space-y-3.5">
              {nearestAgents.map((agent) => (
                <div
                  key={agent.id}
                  className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-205 dark:border-slate-750 shadow-card hover:shadow-hover transition-all flex items-center justify-between gap-4 flex-wrap"
                >
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-right">
                      <p className="text-[9px] text-slate-450 uppercase font-bold">Active Workload</p>
                      <p className="text-xs font-extrabold text-slate-800 dark:text-slate-105 mt-0.5">
